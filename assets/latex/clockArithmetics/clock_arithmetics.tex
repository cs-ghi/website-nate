...
be $7$ o-clock. But if I add $17$ hors to $2$ o-clock, it will \emph{still} be $7$ o-clock. More generally, adding any multiple of
...
\]
So adding $5,(5+1\cdot 12), (5+2\cdot 12), ...$ to $2$ gives the same result. Similarly, if I have any number $n$ between
$1$ and $12$, if I add $b$, or I add $(b + k\cdot 12)$ for some $k = 1,2,...$ I get the same result.

We would like to remember that adding a number $n$ or adding a number $n+k\cdot 12$ to any given hour gives
...
differently, two numbers $a,b$ are equivalent if they give you the same time on the clock-face after addition:
\[
...
between $1$ and $12$ (including $12$), and you'll see that adding $12$ to any number in
clock arithmetic gives back the same
...
\[
  n\cdot 0 = 0
\]
Let us first test if this is the case with an example. First, we would want it so tha
multiplying by $1$ will always give the same time. If it is $12$ o-clock and we add $1$
multiple of $3$, it is $3$ o-clock. If we add $12$ multiples of $3$, then we are adding
$3$ o-clock to itself $12$ times. This brings us $36$ hours forward, which on a clock-face
is $12$ o-clock. But we already saw that we should think of $12$ as being $0$! Hence, we
see that adding $3$ to itself $12$ times will return $0$. In fact, if we start with any
number $n$ between $0$ and $11$, adding the number to itself $12$ times will always bring
us back to the $12$ o-clock position! Therefore, $12$ really \emph{does} behave like $0$
in clock arithmetic! If $\cdot_{12}$ represents multiplication in clock arithmetic, we
just showed that multiplying by $12$ is like multiplying by 0. To go back to our example
using $3$:
\[
  3\Cdot{12}12 = 12
\]
...
\newpage
\section{Application: Solving Equations}

So now that we have defined modular arithmetic, what can we do with it? The principal reason mathematicians care
about modular arithmetic is that it allows us to give a lot more information about equations we are trying
to solve!

For example, consider $x^2+x+1$. A natural question to ask is whether there is an integer $n$ (numbers of the for
...
\]
(look at exercise~\ref{ex:subtraction} if the subtraction is confusing). We can keep $1$ and $0$
...
a square-root in modular arithmetic for some modulo $n$. Let us demonstrate this with an
...
other primes). But
why $11$? Why was the prime $3$ somehow linked to the prime $11$? The reader an check that
...
This is a rather fascinating result: some primes have square-roots in modulo $p$, while
other don't, in other words primes somehow have \emph{relations} between each other! To be
precise, two primes are
tied to each other depending on whether we can take the square root of one modulo the other! Euler in the 19th
...
Here is the final problem we shall present:
  \begin{titledBox*}{}
    ...
    \[
      p\Equiv{5}3
    \]
    In other words, are there finitley many or infiniely many primes for which $p$ o-clock
    ...
    factors).
  \end{titledBox*}
 This question was pondered by Dirichlet in 1837. Essentially is asking about how prime numbers are
 ``distributed'' (i.e., is there a meaningful way in which we can group prime numbers). Just like the
 above problem, I strongly encourage the reader to think about this problem for a couple of hours and just see
 what they can come up with. The result has been proven, and a full proof requires more
 math than what can be covered in an expository article like this (for
     completeness, a proof can be found in~\cite{NateCFT}).
