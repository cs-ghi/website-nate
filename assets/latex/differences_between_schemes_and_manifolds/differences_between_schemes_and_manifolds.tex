...
\begin{enumerate}
  \item The scheme given by $V(x)\cup V(yz) \sse \A_k^3$ has two irreducible components of different
    dimensions. A [connected] manifold (differential and even topological) will always be of a single dimension.
	...
	\item The patches of manifold are very regular, being diffeomorphic to $\R^n$, however patches of a scheme
		...
		connected components. Schemes are often over a certain other ring (and sometimes over schemes) that give
    some regularity; in a geometric setting they are often over fields to parallels varieties.
    ...
    schemes; part of this consequence is the existence of ``large'' points for schemes.

	\item If a scheme $X$ is irreducible, all its open sets are dense. Many schemes
		will be the union of irreducible subschemes where, in each irreducible subscheme, all affine open subsets
    are dense; having only dense open subsets is far from being the case for most manifolds.

  \item More generally, the behaviour of schemes matches more closely with the behaviour of complex geometry
    (for example \emph{Hartog's Extension Lemma} (\cite[chapter~8]{NateComplexAnalysis}) shall have parallels
    with normal schemes, the existence of certain holomorphic/meromorphic functions shall tie into divisor
    theory and Riemann-Roch, compact complex Riemann surfaces shall correspond to projective curves, and so forth).
  ...
  \item A scheme remembers multiplicity (ex. $\Spec k[x]/(x)^2$). A smooth manifold $(M, \CA)$ with maximal
    atlas does not hold this information. \item There can be multiple structure sheaves on the same set
    defining different schemes (think of the different fields that can form the sheaf over a singleton).
    Manifolds of dimension $\le 3$ have unique differential structures up to diffeomorphism.
  ...
  \item the natural additional structure on a smooth manifold is a \emph{tangent bundle} with transition
    ...
    between open sets and fibers, but need to respect a $\GL_n(\R)$-action given by the derivation $D$. There
    is an equivalent \emph{tangent sheaf} for schemes, but more care must be done as schemes are not ``locally
    linear'' everywhere in the way that smooth manifolds must be. Much of the intuition of Poincar\'e's
    duality (see~\cite[section~4.3]{NateAlgTop}) will influence the generalization.
    ...
    As smooth functions on each $U \sse M$ from a sheaf, sheaves that admit a ``partition of unity'' will be
    called \emph{fine sheaves}. A weaker notion of \emph{soft sheaf} will be the closest equivalent to having
    ...
    example, there is no equivalent of $\Spec \Z$, $\Spec \Z[t]$, or $\Spec \CO_K$ (where $\CO_K$ is a ring
    of integers) for smooth manifolds. In this way, scheme theory also allows for a new domain of study that
		is intractable in differential geometry.
