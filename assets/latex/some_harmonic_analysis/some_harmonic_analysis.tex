...
\begin{prop}{Pontryagin Dual}{pontryaginDual}
	Let $G$ be a finite abelian group, $\hat{G} = (\Hom_\Ab(G, \C^*), \cdot)$ the group of characters under
...
This let's us understand the group-operation of $G$ using point-wise function multiplication. Let us now take
a different approach. We can consider all ``continuous'' functions $f: G \to (\C, +)$. Let $G$ have the
discrete topology so that the set of continuous functions from $G$ to $\C$ is all set functions. A-priori,
this collection has no memory of the group operation of $G$. However, it is clearly in bijective
correspondence to the $\C$-vector space $\C[G]$ via the natural mapping:
\[
	\sum_i a_ig_i \mapsto \left(g_i \mapsto a_i\right)_i
\]
We want a $\C$-algebra
isomorphism between $\C[G]$ and $\Hom_\Top(G, \C)$.
This preserves the addition of $\C[G]$ map preserves addition if addition is defined point-wise in the hom-set:
\begin{align*}
...
\end{align*}
which should be intuitive since addition was defined formally. On the other hand, it does \emph{not} preserve
multiplication if the operation is pointwise multiplication:
\begin{align*}
	&\ \Phi\left(\left(\sum_{g \in G} a_ig_i\right)\left(\sum_{g\in G} b_ig_i\right)\right)\\
...
\end{align*}
And it is impossible to define a group-structure on $G$ that will make this an equality unless $G = \{e\}$.
...
Now, as $G\cong \hat{G}$ and $G$ forms an orthonormal basis for $\Hom_\Top(G, \C)$ via indicator functions, it
is natural to ask whether $\hat{G}$ can form such a basis. First, $G$ representing an orthonormal basis is
equivalent to defining ``evaluation'' for $f: G \to \C$ using the usual dot product. To see this, consider:
\[
	...
	\end{cases}
\]
Then for any $f: G \to \C$ there is a unique decomposition (or finite series):
\[
	f = \sum_{h \in G} f(h)\delta_h \qquad \text{so that} \qquad f(g) = \sum_{h \in G} f(h)\delta_h(g)
	= f(g)\cdot 1 = f(g)
\]
The key feature of this basis is that it acts like evaluation when taking the inner product: by taking the
unique inner product (up to isomorphism):
\[
	\langle f,g \rangle = \sum_{h \in G} f(h)\overline{g(h)}
\]
Then $\{\delta_g\}_{g \in G}$ is an orthogonal basis and:
\[
  \langle f, \delta_{g_i} \rangle = \sum_{h\in G} f(h) \cdot \overline{\delta_{g_i}(h)} = f(g_i) \cdot 1 = f(g_i)
	= a_i
\]
this ``evaluation'' behaviour of the inner product with the indicator is because the inner product is just the
dot product, which in this case becomes a projection function
\[
  \langle f, \delta_{g_i} \rangle =  f(g_i)\delta_{g_i}
\]
This is just a special case of Riesz Representation Theorem that says that any linear functional is
inner-product representable using these $\delta_{g_i}$.

Now, the characters $\hat{G}$ are not \emph{a-priori} orthogonal like the indicator functions given by $G$:
one of the key results of harmonic analysis is that they do!
...
As the indicator functionals represent evaluation, the question arises what taking the inner product with the
characters represent. This is most commonly thought thought of as extracting the ``phase'' information.
...
Using the above example, let us change perspective's on how to interpret this change of basis. The
function $f: G \to \C$ can be thought of as taking the ``points'' $g \in G$ and mapping them (continuously) to
the complex numbers. These points are evaluated by the use of a Riesz-representation of $f$ using the
indicator functions that form an orthonormal basis. Bus we just found another orthonormal basis for $\Hom_\Top(G,
\C)$ with which we can take the inner product. We may thus think of the characters $\chi \in \hat{G}$ as
points in their own right: these points are represent ``phases'' information rather than ``location''
information that the indicator functions represent. This motivates the following definition:


\index{Fourier Transform}\index{Gelfand Transform}
\begin{defn}{Fourier Transform (Gelfand Transform)}{fourierTransform}
...
Now, certainly as vector spaces $\Hom_\Top(G, \C)\cong_\C \Hom_\Top(\hat{G}, \C)$ mapping:
\[
  (g \mapsto \langle f, \delta_g \rangle) \mapsto (\chi \mapsto \langle f, \chi \rangle)
\]
But does this map preserve the inner product, and more so can the convolution product of $\Hom_\Top(G, \C)$ be
preserved given the appropriate choice of product on $\Hom_\Top(\hat{G}, \C)$? This answer is yes and can be
thought as the corner-stone result of harmonic analysis on discrete abelian groups:

\index{Fourier Reciprocity}
\begin{thm}{Fourier Reciprocity}{fourierReciprocity}
	The map given in definition~\ref{df:fourierTransform} is a unitary $\C$-algebra isomorphism,
  \[
    (\Hom_\Top(G, \C), +, *) \cong (\Hom_\Top(\hat{G}, \C), +, \cdot)
  \]
  with $\cdot$ in $(\Hom_\Top(\hat{G}, \C), +, \cdot)$ being point-wise multiplication.
\end{thm}

Recall that unitary means $\langle \hat{f}, \hat{g} \rangle = \langle f,g \rangle$.
In a conventional real analysis course:
\begin{itemize}
  \item the $\C$-algebra homomorphism is called the  \emph{Convolution Theorem},
  \item the unitary property is called the \emph{Plancherel's Theorem},
  \item invertibility (i.e. bijectivity) is called the \emph{Fourier Inversion Theorem}.
\end{itemize}
\begin{Proof}
  Some of these are essentially already proven, but for the completeness of this theorem all will be laid
  out.
	\begin{enumerate}
      ...
      &= \frac{1}{|G|} \sum_{h \in G} f(h) \sum_{\chi \in \hat{G}} \chi(gh^{-1})
		\end{align*}
		...
		powers of a certain root of unity) is $|G|$ if $x=e$ and $0$ otherwise. Thus $\sum_{\chi \in \hat{G}}
    \chi(gh^{-1}) = |G|\delta_{g,h}$.
		\[
...
First, take the regular representation of $G$, that is left-multiplication (convolution) $\rho: G \to \GL(\C[G])$ given by:
\[
	\rho(g): f \mapsto g * f
\]
Now, this representation can be broken down into the direct sum of 1-dimensional
irreducible representations. Namely, for a finite abelian group, the set of irreducible representations is
exactly the Pontryagin dual: $\Irr(G) = \widehat{G}$ and so we get:
\[
	\C[G] \cong_\C \bigoplus_{\chi \in \widehat{G}} V_\chi
\]
...
coordinate of $f$ in the irreducible component corresponding to $\chi$. Note that we can define projection
functions $p_\chi = \frac{1}{|G|}\sum_{g \in G}\overline{\chi(g)}g$ and
 in $\C[G]$ these project onto the irreducible subspace $V_\chi$. Convolving any function $f$ with
...
What happens when we wish to analyze functions on abelian groups like $(\R, +)$, $(\Z, +)$, or the circle
group $(S^1, \cdot)$? These are examples of \emph{Locally Compact Abelian (LCA)} groups. Moving from the
finite to the infinite setting requires us to sum on an arbitrary number of elements which introduces the
usual problem of undefinedness. To tame the infinite behaviour, notice that in the finite case we can think of
the delta functions as \emph{measures} so that any $f$ is a sum of weighted singular measures. Thus, we aim to
introduce a natural measure on abelian groups preserving the group structure.
...
\begin{defn}{Haar Measure}{haarMeasure}
	A \emph{left-Haar measure} on an Locally compact (not necessarily abelian) group $G$ is a measure $\mu$ on the Borel $\sigma$-algebra of $G$ that is left-translation-invariant. That is, for any measurable set $E \subseteq G$ and any element $g \in G$:
	\[
		\mu(gE) = \mu(E)
	\]
	This measure is unique up to a positive scaling constant. A similar definition works for the
  \emph{right-Haar measure}. For abelian groups, it is also right-translation-invariant, and it is simply
...
\end{enumerate}
Note that in the infinite case, we can no longer represent points as indicator functions. Instead the
\emph{direct-delta distributions} would take this role; see~\cite{NateRealAnalysis} for more details.
\index{Fourier Reciprocity}
\begin{thm}{Fourier Reciprocity}{fourierReciprocity}
\end{example}
