...
orthonormal basis, or converse given this standard inner product, the indicator function will always be an
orthonormal basis. By Pontryagin duality, $G\cong \hat{G}$, but this group isomorphism does not preserve the
...
multiplication. The representation theory perspective reveals that Fourier
analysis is the simplest case of a universal decomposition principle.
...
\[
	\rho(g): f \mapsto g * f
\]
From this viewpoint, the study of the algebra $(\C[G], *)$ is implicitly a study of the regular
...
exactly the dual group: $\Irr(G) = \widehat{G}$. Thus, we get:
\[
...
coordinate of $f$ in the irreducible component corresponding to $\chi$.

Note that we can define projection functions $p_\chi = \frac{1}{|G|}\sum_{g \in G}\overline{\chi(g)}g$ that we
can construct in $\C[G]$ that project onto the irreducible subspace $V_\chi$. Convolving any function $f$ with
$p_\chi$ annihilates all other ``frequency components'', leaving the part of $f$ that lies in $V_\chi$.

Hence, the Fourier basis \emph{the} unique basis that diagonalizes the natural action of the group on its own algebra of functions.
...
Of course. It's a natural and important step to see how this beautiful, finite theory blossoms into the broader world of harmonic analysis on infinite groups. The following section is designed to follow your proof of Fourier Reciprocity for the finite case, explaining the new tools required for the generalization and how they mirror the concepts we've already established.
...
scaling. This measure is the direct generalization of taking the order of the group, $|G|$.
...
With the Haar measure in hand, we can now redefine our main objects of study by replacing every sum over $G$ with an integral with respect to $d\mu$.
...
\begin{enumerate}
	...
	\item \emph{The Fourier Transform:} The definition of the Fourier transform is the direct analogue of the finite case, replacing the sum with an integral. For a function $f \in L^1(G)$, its Fourier transform is a function $\hat{f}$ on the dual group $\hat{G}$:
	\[
...
\begin{Proof}
    ...
    \]
	This shows that every character $\chi \in \widehat{G}$ defines a maximal ideal. Conversely, it is a
	standard result from the theory of Banach algebras (of which $\mathbb{C}[G]$ is an example) that all such
	maximal ideals arise in this way from homomorphisms into $\mathbb{C}$ (see the \emph{Gelfand-Mazur
	Theorem}), completing the proof.
\end{Proof}

Thus, if convolution algebra $\mathbb{C}[G]$ as our primary object, the natural geometric space that emerges
