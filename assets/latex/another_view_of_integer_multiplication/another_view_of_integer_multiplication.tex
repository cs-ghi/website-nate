...
I want to explore the natural numbers and integers via a formalist philosophy that is not set-theoretical,
...
\begin{itemize}
  ...
  \item vitalization: imposing the existence of an inverse pivoted around an identity. \textcolor{red}{Why is
...
I will try to recover $\N$ by adding restrictions and see if we get something interesting.

Let's consider free commutative semigroup, so group but without necessarily identity or invertibility but
associativity. We keep associativity because order of multiplication can be done ``per pair of prime
divisors'' (see \textcolor{red}{ref:HERE} for the intuition for associativity).

Let's take the free cyclic semigroup generated by a singleton, say $\epsilon$. Then this would give us the
``multiplication'' structure. We can shorten notation and write $\epsilon^n$, which is well-defined notation because
of associativity.
