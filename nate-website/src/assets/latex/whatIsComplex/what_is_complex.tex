...
\newtcbtheorem[auto counter, number within=section]{example}{Exercise}{%
...
deferred the responsibility for its purpose to another question in mathematics. Though each of these has given me
...
\addtolength{\rightskip}{2cm} % Adjust right margin
  Complex analysis is the right completion of polynomials to apply the tools of analysis.
\end{center}
...
As we start in complex analysis with single-dimensional complex-differentiable functions,let us focus on the
special case where there is only one variable. Any polynomial $p(z)$ is naturally studied when over the
complex numbers $\C$: every polynomial over $\C$ breaks down into linear terms:
\[
...
track of multiplicity information} where $c$ is some constant multiple. Characterizing a polynomial via its
roots is done naturally by extending the real to the complex numbers.


It is thus productive to use all the tools of mathematics in understanding the properties of polynomials. We
may continue the study of (single-variable) polynomials over $\C$\footnote{Or more algebraically, over an algebraically closed
...
functions and ask multiple analytic questions (what functionals can we put on them, what type of space do they
form, what are the compact sets of functions). This approach will require a bit more care due to the
``infinite'' nature of analysis as compared to the ``finite'' nature of polynomials. This is where complex
analysis comes to bridge the gap.
...
These two conditions give enough functions for the collection to be closed under uniform convergence on
compact sets (the extra ``on compact sets'' condition is to insure that it is ``locally'' uniformally
continuous\footnote{Technically, being locally uniformally continuous and being uniformally continuous are two
different types of convergence, however they coincide on the space of holomorphic functions}, namely if $f_n$
is a sequence of analytic functions and  $f_n \to f$ uniformally (on compact sets), then $f$ shall still be
analytic by Weiestrass Convergence Theorem. Given a connected open subset $U \sse \C$, we can take all the
polynomials on $U$, and take the all possible uniformally-convergent-on-compact-subset Cauchy-sequences with
the added condition that the resulting function converges on $U$; the collection of all these limits can be
called the \emph{Holomorphic functions}, and we shall denote it as $\CH(U)$. The family $\CH(U)$ forms a
complete metric space\footnote{Even stronger, it forms a Fr\'echet space, which ought to be thought as spaces
  that are not nice enough to define a single norm, but nice enough to replace the norm by a sequence of
compatible semi-norms}, giving us a natural space in which we may ask analytic questions!

Naturally, in complex analysis holomorphic functions are defined as complex-differentiable functions $f: U \to
\C$. What was outlined above is how this collection can be represented by appropriate sequences of polynomials.
...
\begin{itemize}
  ...
  \item If a holomorphic function $f$ is bounded by a polynomial in growth (i.e. $|f(z)| \le |p(z)|$), then
    ...
    lower order or equal to it (recall in first year calculus/analysis where $\lim_{x \to \infty}
    \frac{p(x)}{q(x)}$ was determined by looking at the degree of $p(x)$ and $q(x)$).
  ...
  %      polynomials.
  \item on $\C$, a polynomial is determined by its roots up to a constant multiple. By \emph{Weierstrass factorization
    ...
    In fact, every holomorphic function is determined by its roots up to a multiple of $e^{g(z)}$ where $g(z)$
    is a holomorphic function on $\C$. Note how $e^{g(z)}$ has no roots and yet is a non-constant polynomial.
    The reader should show that if $f(z)$ is an entire holomorphic function that is nowhere zero, then there
    exists a $g(z)$ such that $f(z) = e^{g(z)}$. Thus, $e^{g(z)}$ can be thought of as the ``additional
    constants'' (similarly to how when characterizing a polynomial, we need its roots and an additioanl
    constant $c$: $p(z) = c(z-a_1)\cdots(z-a_n)$).
  ...
  %   particular
    \item Polynomials form an integral domains and rational polynomials form a field. Similarly, the family of
      ...
      From an algebraic point of view, holomorphic functions can be the analytification of \emph{function
      fields}.
...
\]
Through it, we can find many important results about the polynomials that govern the properties of primes. In
fact, one can go so far to say that a large part of the development of modern number theory was the attempt to
find the Riemann-zeta function within the more algebraic domain of number theory (see the \emph{Weil
Conjecture} for more detail). Unfortunately, going into details would require going over the connection
between polynomials and primes, a topic better left for another blog, (or to get the full details
...
incredible results in the theory of polynomials with the help of complex analysis.
...
\begin{example}{Rational Maps and Biholomorphism}{ritionalMapAndBiholo}
  The reader familiar with birational maps may recall they need only be defined on an open dense set. Prove
  that if $f: C \to C'$ is a map between Riemann surfaces that is surjective and is injective on a dense open
  subset, then $f$ is a biholomorphism.
\end{example}
...
appearances!}, and almost no time has been spent elucidating the bridges between algebra and geometry and its
connection to complex analysis (which is where higher-dimensional complex analysis naturally shows up in
scheme theory)\footnote{The connection to Riemann surfaces shows a small part of this connection, showing how
  a crude invariant of curves is their genus number, however the Riemann-Roch theorem had no time to make an
appearance!}. The reader interested in seeing the details of these ideas expanded upon can checkout the
multitude of textbook on complex analysis, or you can checkout~\cite{NateComplexAnalysis} and\cite{NateAlgGeo}
to see my exposition on the topics.
